             ...
             bool               inLog)
{
   ...
   _carte = SparseMatrix<Human *>(nullptr, _size, _size);
   _carte.set_size(_size, size);

   std::cout << _carte.getHeight() << " " << _carte.getWidth() << std::endl;
   /*
   ...
   _carteLieu = SparseMatrix<char>(0, _size, _size);
   _carteLieu.set_size(_size, size);

   std::cout << _carteLieu.getHeight() << " " << _carteLieu.getWidth() << std::endl;
   /*
   ...
   for(City* c : _listVille)
   {
      ...
      for(int i = 0; i<c->getNbHab();i++)
      {
         ...
         { 
            ...
            row = (rng()%c->getTaille()) + c->getCoordX();
            column = (rng()%c->getTaille()) + c->getCoordY();
            varEmpty = isEmpty(row,column);
         }


         _carte.add(new Human(inSimulationParams,row,column), row,column);
         ...
         tempCompt--;
         float randValue = rng() * (1.0 / 4294967295.0);
         if(randValue < _tauxObesitePopulation)
         {
            _carte(row,column)->setIsObese();
         }
         
         randValue = rng() * (1.0 / 4294967295.0);
...
/*      
      ...
      {
         float randValue = rng() * (1.0 / 4294967295.0);
         float randValueRappel = rng() * (1.0 / 4294967295.0);
...
/*
   for(int qtyAgents = 0; qtyAgents<_nbMalade;qtyAgents++)
   {
      ...
      index = rng()%_humanSafePositions.size();
...
void World::contamination(int inRow, int inColumn, int inCurrentRow, int inCurrentColumn)
{
   ...
   for(Position  pos : target_v1["human"])
   {
      ...
      if(!_carte(pos.getPosX(),pos.getPosY())->isSick())
      {
         ...
         float randomValue = rng() * (1.0 / 4294967295.0);
                          ...
                          bool inDie)
{
   ...
   if(!inDie)
   {   
      _carte.add(_carte(inFromRow,inFromColumn),inToRow,inToColumn);
      _carte(inToRow,inToColumn)->setPosition(inToRow,inToColumn);
      _carte.remove(inFromRow,inFromColumn);
...
void World::moveHumanSafe(int inRow, int inColumn)
{
   ...
   double probaMoveOutsideCity  = rng() * (1.0 / 4294967295.0);
   ...
   if(_carte(inRow,inColumn)->getVille()->getTaille() > 0 && probaMoveOutsideCity > 0.25)
   {
      ...
      if(_isDeplacementLimites)
      {
         ...
         {
            rowDeplacement = (inRow-_nbDistanceMax) + (((inRow + _nbDistanceMax) - (inRow-_nbDistanceMax)) * (rng() * (1.0 / 4294967295.0))); 
            columnDeplacement = (inColumn-_nbDistanceMax) + (((inColumn + _nbDistanceMax) - (inColumn-_nbDistanceMax)) * (rng() * (1.0 / 4294967295.0))); 
         }
      ...
      if(_isConfinement && (_nbLimiteDistanceMaxConfinement < _nbDistanceMax))
      {
         ...
         while(rowDeplacement < coordX || rowDeplacement > coordX+taille-1 || columnDeplacement < coordY || columnDeplacement < coordY+taille-1)
         {
            rowDeplacement = (inRow-_nbLimiteDistanceMaxConfinement) + (((inRow + _nbLimiteDistanceMaxConfinement) - (inRow-_nbLimiteDistanceMaxConfinement)) * (rng() * (1.0 / 4294967295.0))); 
            columnDeplacement = (inColumn-_nbLimiteDistanceMaxConfinement) + (((inColumn + _nbLimiteDistanceMaxConfinement) - (inColumn-_nbLimiteDistanceMaxConfinement)) * (rng() * (1.0 / 4294967295.0)));
         }
      ...
      else
      {
         ...
         rowDeplacement = (rng()%taille) + coordX;
         columnDeplacement = (rng()%taille) + coordY; 
      }
   ...
   else
   {
      ...
      else
      {
         ...
         rowDeplacement = rng()%_size; 
         columnDeplacement = rng()%_size; 
      }  
   }
   ...
   if(target_v1["empty"].size() != 0)
   {
      ...
      int randomValue = ((long)floor(rng()))%taille;
      humanGoFromTo(inRow,
                    inColumn, 
...
void World::startSimulation(SimulationParams * inSimulationParams)
{
       ...
       iteration++)
   {
      //displayStats();
      //display();
      //writeLog("TestPRN:" + to_string(rng() * (1.0 / 4294967295.0)));
      //World::pause();
      nextIteration();
      /*
